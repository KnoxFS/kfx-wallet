...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
		...
		LogPrintf("reward idx is: %d\n", reward_out_idx);
                ...
                //txReward.vout[reward_out_idx].nValue = block_value;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("KnoxFSMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("KnoxFSMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("KnoxFSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kfx-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running KnoxFSMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
