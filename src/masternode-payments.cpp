...
bool CMasternodePayments::MNPaymentDue(int nHeight) {
    ...
    for(unsigned mnlevel = CMasternode::LevelValue::MIN; mnlevel <= CMasternode::LevelValue::MAX; ++mnlevel) {
        ...
        CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, mnlevel, 10 * COIN);
