...
#if defined(NDEBUG)
#error "KnoxFS cannot be compiled without assertions."
#endif
...
const int nVersionFork = 1;
const int nVersionSecondFork = 2;
...
/** Fees smaller than this (in ukfx) are considered zero fee (for relaying and mining)
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasKFXInputs = !lnoInputs.empty();
