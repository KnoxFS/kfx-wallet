...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a KnoxFS address (e.g. %1)").arg("KH2ABNnnXUpSYxAR9k7sYdXkfUmZ4FMKkn"));
#endif
...
{
    // return if URI is not valid or is no KnoxFS: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert kfx:// to kfx:
    //
    //    Cannot handle this later, because kfx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open kfx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KnoxFS.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for KnoxFS.lnk
...
{
    return GetAutostartDir() / "kfx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a kfx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=KnoxFS\n";
